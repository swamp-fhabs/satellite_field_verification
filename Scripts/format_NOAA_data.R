...
find_sampling_pixels <- function(pix.center.sf, field.lat.long, utm){
  ...
  data.combined.distinct <- as_tibble(data.combined) %>% 
    select(date, waterbody, pix_site, pixID, starts_with("rhos")) %>% 
...
calc_CI <- function(df){
  ...
  # From Lunetta et al. 2015
  CIcyano_sat <- ifelse(ss665_sat > 0, CI_sat, 0)
...
field.CI.list <- field_CI_values %>% 
  group_by(waterbody) %>% 
  group_split(.)
names(field.CI.list) <- unique(field_CI_values$waterbody)
field.CI.list2 <- field.CI.list[c(6, 1, 7, 5, 2, 3)]


field.sat.df <- map2(field.CI.list2, ci_sat_list, function(x, y) left_join(x, select(y, -waterbody), by= "site")) %>% 
  bind_rows()
