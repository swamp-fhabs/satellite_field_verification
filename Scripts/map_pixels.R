...
# LAKE SAN ANTONIO 
lsa_lake_map <- plot_whole_lake(data_list = lsa_data, shapefile_utm = lsa_utm, scalebar_dist = 2000)
lsa_bbox_map <- plot_bounding_box(data_list = lsa_data, shapefile_utm = lsa_utm, bbox= lsa_bbox)


# SAN PABLO RESERVOIR
spr_lake_map <- plot_whole_lake(data_list = spr_data, shapefile_utm = spr_utm, scalebar_dist = 1000)
spr_bbox_map <- plot_bounding_box(data_list = spr_data, shapefile_utm = spr_utm, bbox= spr_bbox)
...
#   scalebar(data= lsa_utm, dist= scalebar_dist, dist_unit = "m", location= "bottomleft", transform= FALSE, st.bottom= FALSE, 
#            st.dist = 0.02,
#            st.size= 3, 
#            border.size = 0.5) +
#   labs(x= "Longitude", y= "Latitude") +
